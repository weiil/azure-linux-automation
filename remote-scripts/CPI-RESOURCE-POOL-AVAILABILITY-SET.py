...
def Update_cf_manifest(source_yml_file, destination_yml_file, availability_set_name):
    ...
    out['resource_pools'][0]['cloud_properties']['availability_set'] = availability_set_name
    out['name'] = cf_deployment_name
    out['jobs'][0]['name'] = 'availabilityset_z1'
...
def RunTest():
    ...
    Update_cf_manifest(source_manifest_name, destination_manifest_name, availability_set_name)
    if DeployCF(destination_manifest_name):
        RunLog.info('Start to verify the availability set')
        host_name = Run("bosh vms --details | grep availabilityset_z1 | awk '{print $13}'")
        vm_info = Run('az vm show -g %s -n %s' % (resource_group_name, host_name.strip('\n')))
        data = json.loads(vm_info)
        availabilitySet = data['availabilitySet']['id']
        if availability_set_name in availabilitySet.lower():
...
destination_manifest_name = 'cpi-availability-set-cf.yml'
cf_deployment_name = 'availability-set-cf'
availability_set_name = 'test-availability-set'
