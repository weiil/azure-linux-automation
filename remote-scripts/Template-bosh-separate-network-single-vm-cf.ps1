...
try
{
	...
	Remove-Item *.yml
	$cfScenario = 'separate-network-single-vm-cf'
	...
	$out = echo y | .\tools\plink -i .\ssh\$sshKey -P $port $dep_ssh_info "python cf-manifest-handle.py example_manifests/single-vm-cf.yml $yml_file $new_cfip && echo yml_update_pass || echo yml_update_fail"
...
"@
		...
		if ($out -match "cf_deploy_ok")
		{
			...
			[String]$vmInfo = echo y | .\tools\plink -i .\ssh\$sshKey -P $port $dep_ssh_info "bosh ssh separatenetwork_z1/0 hostname"
			if($vmInfo -match "separatenetwork_z1/0\s+(\S+)\s+Cleaning")
			{
				$vmName = $Matches[1]
				$nic = Get-AzureRmNetworkInterface -ResourceGroupName $isDeployed[1] | where {$_.VirtualMachine.Id.EndsWith($vmName)}
				$publicIPAddressInfo = $nic.IpConfigurations[0].PublicIpAddress.Id
